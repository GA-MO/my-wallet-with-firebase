  ...
  render() {
    ...
    return (
      <div className="container">
        <div className="row">
          <div className="D-8">
            <Input fluid type='text' placeholder='Price...' size='massive' action>
              <input onChange={(e) => this.setState({ inputPrice: e.target.value })} value={inputPrice} />
              <Select onChange={(e, { value }) => this.setState({ inputCategory: value })} compact options={categorySelect} placeholder='Category' value={inputCategory} />
              <Button content='Add' size='massive' color="teal" onClick={() => this.addItem()} />
            </Input>
          </div>
          <div className="D-4">
            <BuyList
